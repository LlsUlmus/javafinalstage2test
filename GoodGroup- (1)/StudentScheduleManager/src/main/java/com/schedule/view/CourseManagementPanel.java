...
import com.schedule.util.ExcelUtil;
...
public class CourseManagementPanel extends JPanel {
    ...
    private void importFromExcel() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            List<Course> courses = ExcelUtil.importCoursesFromExcel(filePath, currentUser.getId());

            if (courseController.batchAddCourses(courses)) {
                JOptionPane.showMessageDialog(this, "导入成功", "成功", JOptionPane.INFORMATION_MESSAGE);
                loadCourses();
            } else {
                JOptionPane.showMessageDialog(this, "导入失败", "错误", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
}
